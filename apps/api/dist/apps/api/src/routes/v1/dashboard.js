 ...
 * Validation schemas
 */
const timeframeSchema = zod_1.z.object({
    timeframe: zod_1.z.enum(['today', 'week', 'month', 'quarter', 'year', 'custom']).optional().default('month'),
 ...
 */
const calculateDashboardMetrics = async (consultantId, dateRange) => {
    ...
    const abandonedSessions = sessions.filter((s) => s.status === 'CANCELLED').length;
 ...
 */
const generateChartData = async (consultantId, dateRange) => {
    const prisma = (0, database_1.getPrismaClient)();
    const { start, end } = dateRange;
 ...
 */
router.get('/metrics', (0, validation_1.validateRequest)(timeframeSchema, 'query'), async (req, res) => {
    try {
        ...
        // Cache for 5 minutes
        await redis_1.cacheUtils.setWithAutoTTL(cacheKey, metrics, 'shortCache');
        res.json({
            data: metrics,
            fromCache: false,
            dateRange: {
 ...
 */
router.get('/charts', (0, validation_1.validateRequest)(timeframeSchema, 'query'), async (req, res) => {
    try {
        const { timeframe, startDate, endDate } = req.query;
        const consultantId = req.user.id;
        // Check cache first
        const cacheKey = `dashboard_charts:${consultantId}:${timeframe}:${startDate || ''}:${endDate || ''}`;
        const cachedCharts = await redis_1.cacheUtils.get(cacheKey);
        if (cachedCharts) {
            res.json({
                data: cachedCharts,
                fromCache: true
        ...
        await redis_1.cacheUtils.set(cacheKey, chartData, 600);
        res.json({
            data: chartData,
            fromCache: false
        });
...
router.get('/recent-activity', async (req, res) => {
    try {
        ...
        const cachedActivity = await redis_1.cacheUtils.get(cacheKey);
        if (cachedActivity) {
            res.json({
                data: cachedActivity,
                fromCache: true
        ...
        const sortedActivities = activities
            ...
            .slice(0, limit);
        // Cache for 2 minutes
        await redis_1.cacheUtils.set(cacheKey, sortedActivities, 120);
        res.json({
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const cachedSummary = await redis_1.cacheUtils.get(cacheKey);
        if (cachedSummary) {
            res.json({
                data: cachedSummary,
                fromCache: true
        ...
        const [totalSessions, totalClients, totalRevenue, pendingSessions] = await Promise.all([
        ...
        // Cache for 3 minutes
        await redis_1.cacheUtils.set(cacheKey, summary, 180);
        res.json({
            data: summary,
            fromCache: false
        });
...
function getQuotationAction(status) {
    switch (status) {
