...
const generateTokens = async (user, userType) => {
        ...
        try {
            ...
            if (error.code === 'P2002' && error.meta?.target?.includes('token')) {
                ...
                if (retryCount < MAX_RETRIES) {
                    ...
                    logger_1.logger.warn(`Refresh token collision detected, retrying... (${retryCount}/${MAX_RETRIES})`);
                    continue;
                }
                else {
                    logger_1.logger.error('Max retries exceeded for token generation:', error);
                    throw new appError_1.AppError('Failed to generate unique refresh token after multiple attempts', 500);
                }
            }
    ...
    throw new appError_1.AppError('Token generation failed - unexpected error', 500);
};
